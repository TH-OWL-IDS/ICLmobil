...
<template>
  <f7-sheet @sheet:opened="init"
            ...
            class="ride-booking-own-vehicle">
  <template #fixed>
...
<script>
  ...
  export default {
      ...
      created() {
          this.emitter.on('set-ride-booking-own-vehicle', (options) => {
      ...
      methods: {
        init() {
          console.log("SHEET RIDE BOOKING OWN VEHICLE OPENED!")
          this.drawMap();
        ...
        async getDirections(start, end) {
          try {
              ...
              let tripMode = this.selectedRideOption.vehicleType === 'scooter' ? 'own_scooter' : 'own_bike'; 
              let tripData = {
                trip_mode:  tripMode,
          ...
          try {
            if (this.phone_unverified || this.email_unverified) {
              ...
              let data = {
                ...
                start_time: this.selectedRideOption.start_time,
                end_time: this.selectedRideOption.end_time,
        ...
        async reset() {
          return new Promise((resolve) => {
            ...
            f7.sheet.close('.ride-booking-own-vehicle');
            setTimeout(() => {
              resolve();
            }, 350);
          });
        ...
        openSheet() {
          f7.sheet.open('.ride-booking-own-vehicle');
        },
        closeSheet() {
      ...
      beforeUnmount() {
          this.emitter.off('set-ride-booking-own-vehicle');
      },
  };
</script>
<style scoped>
