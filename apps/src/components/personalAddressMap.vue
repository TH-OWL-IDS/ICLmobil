    ...
    <f7-sheet @sheet:opened="init"
        ...
        <f7-toolbar class="sheet-toolbar-top">
            <div class="left sheet-title">
                {{ this.$t('account.sheet.homeadress.title') }}
            </div>
            <div class="right">
        ...
        <f7-toolbar class="sheet-toolbar">
            <div class="searchbar searchbar-inner sheet-searchbar">
                <div class="searchbar-input-wrap">
                    <input type="search" 
                        :placeholder="this.$t('account.sheet.homeadress.map.search.placeholder')"
                        @input="onInput" 
                        @focus="onFocus"
                        @keyup.enter="onEnter"
                        v-model="searchQuery" />
        ...
        <f7-page>
            ...
            <div class="button-wrapper">
                <f7-button href="#" :disabled="isLocalFavoriteEmpty" @click="saveAddress" fill large style="--f7-button-large-text-transform: none"><f7-icon ios="f7:checkmark_alt_circle_fill" md="material:check_circle"></f7-icon>&nbsp;{{ this.$t('account.sheet.homeadress.button.saveaddress') }}</f7-button>
            </div>
        </f7-page>
    </f7-sheet>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            init() {
                console.log("SHEET HOME ADRESS OPENED");
                this.drawMap();
                this.resetSearch();
            ...
            drawMap() {
                ...
                this.geoLocator.on('error', async (position) => {
                    this.mapDisabledText = this.$t('account.sheet.homeadress.map-not-available');
                    this.closeToastWait();
                });
            ...
            async handlePickResult(result) {
                try {
                    ...
                    let markerIcon = 'fa-solid fa-house';
                    ...
                    const index = this.favorites.findIndex(favorite => favorite.type === 'Home');
                    if (index !== -1) {
            ...
            saveAddress() {
                ...
                if (index !== -1) {
                    ...
                    console.log(`Home updated:`, this.favorites[index]);
                } else {
                    this.favorites.unshift(this.localFavorite);
                    console.log(`Home added:`, this.localFavorite);
                }
            ...
            closeSheet() {
                ...
                f7.sheet.close('.personalAddress');
            },
